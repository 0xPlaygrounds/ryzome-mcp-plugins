...
- `packages/hermes-ryzome` (`ryzome-hermes-plugin` on PyPI) — Hermes plugin adapter. Python plugin that bundles a compiled Node runner invoking `ryzome-core` tools. Not published to npm (the internal npm name `@ryzome-ai/hermes-ryzome` is marked `private`).
