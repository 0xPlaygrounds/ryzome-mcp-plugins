...
type OpenClawConfig = {
  plugins?: {
    entries?: Record<string, { enabled?: boolean; config?: Record<string, unknown> } | undefined>;
...
async function executeCreateCanvasTool(stateDir: string, config: OpenClawConfig, title: string) {
  const installedPluginRoot = path.join(stateDir, "extensions", "openclaw-ryzome");
  const installedEntry = path.join(installedPluginRoot, "src", "index.ts");
  const pluginModule = await withStateDirEnv(stateDir, async () =>
    await import(pathToFileURL(installedEntry).href),
  );
...
async function installPackagedPlugin(stateDir: string) {
  ...
  const installOutput = runOpenClaw(["plugins", "install", tarballPath], stateDir);
...
describe("OpenClaw integration", () => {
  it("installs the packed plugin, configures it, and executes a stubbed canvas run", async () => {
    ...
    try {
      ...
      expect(configAfterInstall.plugins?.entries?.["openclaw-ryzome"]?.enabled).toBe(true);
      expect(configAfterInstall.plugins?.installs?.["openclaw-ryzome"]).toBeTruthy();
      ...
      expect(configuredConfig.plugins?.entries?.["openclaw-ryzome"]?.config).toMatchObject({
        apiKey: "stub-api-key",
        apiUrl: stub.apiUrl,
        appUrl: stub.appUrl,
      });
